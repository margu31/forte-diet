...
export default function MealModalContainer({ onMealModal, mealListData, showMealModal }) {
  ...
  const mealIdIndex = mealsArrayList.map(data => parseInt(data.id)).indexOf(parseInt(id));
  ...
  const mealArrayIndex = mealIdIndexArray.filter(data => data === mealIdIndex)[0];

  const [currentMealArrayIndex, setCurrentMealArrayIndex] = useState(mealArrayIndex);

  // 캐러셀 전체 길이
  const mealLength = mealsArrayList.map(data => data.id).length - 1;

  const prevMeal = e => {
    setCurrentMealArrayIndex(currentMealArrayIndex === 0 ? mealLength : currentMealArrayIndex - 1);
  };

  const nextMeal = e => {
    setCurrentMealArrayIndex(currentMealArrayIndex === mealLength ? 0 : currentMealArrayIndex + 1);
  };
